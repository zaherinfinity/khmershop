...
export default function Home() {
  ...
  const handleFind = async () => {
    ...
    try {
      ...
      setStats(prev => ({ ...prev, targets: data.targets?.length || 0 }));
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setStats(prev => ({ ...prev, apis: data.apis?.length || 0 }));
  ...
  const handleParams = async () => {
    ...
    try {
      ...
      setStats(prev => ({ ...prev, params: data.params?.length || 0 }));
  ...
  // ---- Live Check (for finder) ----
  const checkLive = async () => {
    if (!finderData.length) return toast('No targets to check', 'error');
    toast(`Checking ${finderData.length} targets...`, 'info');
    try {
      const res = await fetch('/api/check-live', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ urls: finderData })
      });
      const { live } = await res.json();
      setStats(prev => ({ ...prev, live: live.length }));
      toast(`${live.length}/${finderData.length} live`, 'success');
    } catch (e) {
      toast('Error checking live status', 'error');
    }
  };
  ...
  const clearAll = () => {
    ...
    setBatchResult([]);
    setBatchItems([]);
    setStats({ targets: 0, live: 0, params: 0, apis: 0 });
    toast('All cleared', 'info');
  };

  return (
    <div style={{ fontFamily: 'Inter, sans-serif', background: '#0b0d15', color: '#f0f4ff', minHeight: '100vh', position: 'relative' }}>
      <canvas ref={canvasRef} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0, pointerEvents: 'none' }}></canvas>
        ...
        <header style={{ padding: '18px 24px', borderBottom: '1px solid rgba(255,255,255,0.04)', background: 'rgba(11,13,21,0.75)', backdropFilter: 'blur(20px)' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
              ...
              <button onClick={clearAll} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 100, padding: '6px 18px', color: '#a8b5d9', cursor: 'pointer' }}>
                ...
                <div style={{ marginTop: 24 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', marginBottom: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button onClick={checkLive} style={{ padding: '6px 16px', borderRadius: 10, border: 'none', background: '#00b894', color: '#fff', cursor: 'pointer' }}><i className="fas fa-heartbeat"></i> Check Live</button>
                    </div>
                  </div>
